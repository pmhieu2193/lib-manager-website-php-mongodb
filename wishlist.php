...
<body>
    ...
    <h2>Danh sách yêu thích<h2>
            <div class="small-container cart-page">
                <a class="back" onclick="location.href='index.php'">&larr; Mượn thêm sách khác vào danh sách yêu thích</a><br>
                        ...
                        <table>
                            ...
                            foreach ($wishList as $doc) {
                                ...
                                if (!$sach) {
                                    ...
                                    echo '</td>
                                    <td>
                                        <a href="book.php?_id=' . (string)$sach->_id . '" class="link-text">Xem Chi Tiết</a>
                                        <br>
                                        
                                        <form action="wishlist.php" method="post">
                                        <input type="hidden" name="book_remove" value="' . $sach->_id . '">
                                        <button type="submit" class="btn-remove" name="btn_remove">Xoá sách khỏi giỏ</button>';
                                    if ($sach->so_luong > 0 && $sach->trang_thai_sach != 0) {
                                        echo '</form>
                                            <form action="xu_ly_sach.php" type="post">
                                            <input name="id_book" type="hidden" value="' . $sach->_id . '"><button type="submit" name="addToCart" class="btn cart-btn">Thêm vào giỏ sách</button>
                                            </form>';
                                    }
                                    echo '
                                    </td>
