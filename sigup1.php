<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($sigup) {;
                header("Location: sigup.php?error=Email da dc su dung");
                exit();
        }
        $sigup1 = $collection->findOne(['cccd'=> $cccd]);
        if ($sigup1) {;
            header("Location: sigup.php?error=CCCD nay da dang ky truoc do");
            exit();
        }
        ...
        if ($insertOneResult->getInsertedCount() > 0) {
            ...
            header("Location: login.php?error=Tao tai khoan thanh cong, vui long dang nhap");
            exit();
          } else {
            header("Location: sigup.php?error=Tao tai khoan that bai, vui long thu lai sau!");
            exit();
          }
